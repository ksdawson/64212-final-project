...
def create_traj_db(iiwa1_config, iiwa2_config):
    ...
    # Each has 3 trajectories:
    # (1) home -> pre-pick -> pick
    # (2) pick -> post-pick -> home
    # (3) home -> pre-place -> place
    # TODO for capturing: (4) home -> table and (5) table -> home

    # Create a game
    game = Game()

    # Compute all 3 trajectories for each square
    ...
    for file_idx in range(ord('a'), ord('h') + 1):
        ...
        for rank_idx in range(1, 8 + 1):
            ...
            pick_xyz = X_WG_base.translation()
